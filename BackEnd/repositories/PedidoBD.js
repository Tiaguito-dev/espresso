...
const selectPedidoPorId = 'SELECT * FROM producto WHERE id = $1'; // En la base de datos lo tuve uqe llamar id pero la idea es que se llame codigo, PORQUE EN TODOS LADOS LE PUSIMOS ID LA PUTA MADRE
...
exports.obtenerPedidoPorId = async (nro_pedido) => {
    try {
        const pedidos = await Gateway.ejecutarQuery({ text: selectPedidoPorId, values: [nro_pedido] });
        //HAY QUE DEFINIR SI LO VA A BUSCAR POR NOMBRE O POR CODIGO, Porque de eso depende también que sea unique o no
        return pedidos[0]; // Retornar el primer producto encontrado
    } catch (error) {
