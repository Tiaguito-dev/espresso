...
=======
const selectProductoPorId = 'SELECT * FROM producto WHERE id = $1'; // En la base de datos lo tuve uqe llamar id pero la idea es que se llame id, PORQUE EN TODOS LADOS LE PUSIMOS ID LA PUTA MADRE
>>>>>>> avi-rama
...
// Esto en realidad sería por id, pero en la bd lo puse id
...
exports.crearProducto = async (datosDeProducto) => {
    ...
    const { id, precio, nombre, descripcion, categoria } = datosDeProducto;

    try {
        await Gateway.ejecutarQuery({ text: insertProducto, values: [id, precio, nombre, descripcion, categoria] });
        return {
            success: true,
